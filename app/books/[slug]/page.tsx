...
export default async function BookDetailPage({ params }: PageProps) {
  ...
  const { data: book } = await supabase
    .from("books")
    .select(
      "id, title, subtitle, slug, short_description, description, cover_image, isbn, language, page_count, genre, format, amazon_link, flipkart_link, publisher_link, purchase_link_international, purchase_link_pothi, purchase_link_library, published_date, featured, created_at, author:authors(id, name, slug, bio, image_url)"
    )
    .eq("slug", slug)
    .single();
  ...
  return (
    <>
      ...
      {book.purchase_link_library && (
        <a
          href={book.purchase_link_library}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2.5 w-full py-3.5 px-6 rounded-xl font-bold text-sm text-gray-900 bg-white border-2 border-gray-900 hover:bg-gray-50 transition-colors"
        >
